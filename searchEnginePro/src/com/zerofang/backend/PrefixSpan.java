...
import com.app.backend.utils.PairData;
import com.app.backend.utils.ReadFile;
...
public class PrefixSpan {
	...
	public void project(PairData projected) throws IOException {
		...
		for (int i = 0; i < dataBase.size(); i++) {
			...
			for (int iter = projected.indeces.get(i); iter < itemSet.size(); iter++) {
				...
				if (mapItem.get(itemSet.get(iter)) != null) {
					mapItem.put(itemSet.get(iter),
							mapItem.get(itemSet.get(iter)));
				} else {
		...
		// for (map<unsigned int, unsigned int>::iterator it_1 =
		// map_item.begin(); it_1 != map_item.end(); it_1++) {
